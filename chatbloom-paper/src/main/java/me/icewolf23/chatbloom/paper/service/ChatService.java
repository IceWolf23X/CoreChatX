...
public final class ChatService {
    ...
    public void handlePublicChat(Player sender, String rawMessage, String channelId) {
        ...
        ProcessedMessage rendered = processSanitizedMessage(sender, sanitized, channel.allowChatItems(), channel.allowMentions());
        ...
        Component finalMessage = plugin.formats().publicChat(sender, rendered.component(), channel);
        ...
        plugin.bridges().dispatchChannelMessage(channel, sender.getName(), rendered.plainText());
        if (channel.scope() == ChannelScope.NETWORK
            && plugin.configs().deployment().networkFeaturesAllowed()
            && plugin.services().networkBridge().isEnabled()) {
    ...
    public void handleRemoteNetworkChat(ChatMessagePacket packet) {
        if (!plugin.configs().deployment().networkFeaturesAllowed()) {
            return;
        }
        ...
        ProcessedMessage rendered = processRemoteSanitizedMessage(packet.plainText(), channel.allowMentions());
        if (rendered.plainText().trim().isEmpty()) {
            return;
        }
        Component finalMessage = plugin.formats().publicChatRemote(packet.senderName(), packet.rankPrefixTemplate(), rendered.component(), channel);
    ...
    public ProcessedMessage processSanitizedMessage(Player sender, String sanitized) {
        return processSanitizedMessage(sender, sender, sanitized, true, true, false);
    }

    public ProcessedMessage processSanitizedMessage(CommandSender sender, String sanitized, boolean allowChatItems, boolean allowMentions) {
        return processSanitizedMessage(sender, sender instanceof Player player ? player : null, sanitized, allowChatItems, allowMentions, false);
    }

    public ProcessedMessage processSanitizedMessage(Player sender, String sanitized, boolean allowChatItems, boolean allowMentions) {
        return processSanitizedMessage(sender, sender, sanitized, allowChatItems, allowMentions, false);
    }

    public ProcessedMessage processRemoteSanitizedMessage(String sanitized, boolean allowMentions) {
        return processSanitizedMessage(null, null, sanitized, false, allowMentions, true);
    }

    private ProcessedMessage processSanitizedMessage(CommandSender sender, Player playerContext, String sanitized, boolean allowChatItems, boolean allowMentions, boolean remoteSender) {
        ...
        for (String token : splitPreservingWhitespace(sanitized)) {
            ...
            TokenRender tokenRender = renderCoreToken(sender, playerContext, decorated.core(), allowChatItems, allowMentions, remoteSender, onlinePlayers, notificationTargets, bypassTargets);
    ...
    private TokenRender renderCoreToken(
        ...
        CustomPingDefinition customPing = allowMentions ? customPings.get(normalized) : null;
        ...
        if (allowMentions && mentionsEnabled) {
