...
public final class PlayerDataStore {
    ...
    public void reload() {
        ...
        for (String rawUuid : configuration.getConfigurationSection("players").getKeys(false)) {
            ...
            settings.setMentionNotificationsEnabled(configuration.getBoolean(base + "mention-notifications", true));
            settings.setStaffChatEnabled(configuration.getBoolean(base + "staff-chat-enabled", true));
            settings.setLocaleTag(configuration.getString(base + "locale", "en_us"));
    ...
    public void save() {
        ...
        for (Map.Entry<UUID, PlayerSettings> entry : cache.entrySet()) {
            ...
            configuration.set(base + "mention-notifications", settings.isMentionNotificationsEnabled());
            configuration.set(base + "staff-chat-enabled", settings.isStaffChatEnabled());
            configuration.set(base + "locale", settings.getLocaleTag());
        }
        try {
    ...
    private PlayerSettings createDefaults() {
        ...
        settings.setMentionNotificationsEnabled(true);
        settings.setStaffChatEnabled(true);
        settings.setLocaleTag("en_us");
        return settings;
    }
}
