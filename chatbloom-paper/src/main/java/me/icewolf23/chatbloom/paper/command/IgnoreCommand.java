...
public final class IgnoreCommand implements TabExecutor {
    ...
    public IgnoreCommand(ChatBloom plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("chatbloom.command.ignore")) {
        ...
        if (target.getUniqueId().equals(player.getUniqueId())) {
            player.sendMessage(plugin.formats().configMessage("privacy.ignore-self", player));
            return true;
        }
        ...
        player.sendMessage(plugin.formats().configMessage("privacy.ignore-added", player, Placeholder.unparsed("target_name", target.getName() == null ? args[0] : target.getName())));
        return true;
    }
