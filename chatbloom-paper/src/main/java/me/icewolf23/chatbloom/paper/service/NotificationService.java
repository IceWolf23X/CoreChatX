...
public final class NotificationService {
    ...
    public void reload() {
        ...
        this.pingSound = sound(messages.getString("ping.notification-sound", keyString(Sound.ENTITY_EXPERIENCE_ORB_PICKUP)), Sound.ENTITY_EXPERIENCE_ORB_PICKUP);
        ...
        this.privateMessageSound = sound(messages.getString("private-messages.received-sound", keyString(Sound.BLOCK_NOTE_BLOCK_BELL)), Sound.BLOCK_NOTE_BLOCK_BELL);
    ...
    private Sound sound(String input, Sound fallback) {
        ...
        NamespacedKey key = NamespacedKey.fromString(normalizedInput.toLowerCase(Locale.ROOT));
        if (key != null) {
            Sound resolved = Registry.SOUND_EVENT.get(key);
            if (resolved != null) {
                return resolved;
            }
        }
        ...
        if (!normalizedInput.contains(":")) {
            String fieldName = normalizedInput.toUpperCase(Locale.ROOT).replace('.', '_');
            try {
                Field field = Sound.class.getField(fieldName);
                Object value = field.get(null);
                if (value instanceof Sound resolved) {
                    return resolved;
                }
    ...
    private String keyString(Sound sound) {
